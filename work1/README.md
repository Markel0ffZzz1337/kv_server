...
本项目没有依赖任何第三方网络框架，完全基于 Python 原生的 socket 和 threading 模块实现。服务器采用**主线程监听、子线程处理**的并发模型，并将数据存储在内存字典中。
...
_(在此处插入你的服务器终端成功运行并处理请求的截图)_

请将截图存放在 images 文件夹中，并将此行替换为例如：!\[服务器运行截图\](images/server_run.png)

### 2\. JMeter 并发性能测试结果

_(在此处插入你的 JMeter 汇总报告截图，展示 TPS 和 Average 等指标)_

请将截图存放在 images 文件夹中，并将此行替换为例如：!\[JMeter测试结果\](images/jmeter_result.png)